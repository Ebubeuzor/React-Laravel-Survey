...
export default function QuestionEditor({
  ...
  function deleteOption(op) {
    model.data.option = model.data.option.filter(opt => opt.uuid != op.uuid)
    setModel({...model})
  }
  ...
  function onTypeChange(ev) {
    ...
    if (!shouldHaveOptions(model.type) && shouldHaveOptions(ev.target.value)) {
        if (!model.data.option) {
            newModel.data = {
              option: [{ uuid: uuidv4(), text: "" }],
            };
        }
    }
    setModel(newModel);
  ...
  return (
    ...
    <div>
      ...
      <div className="mb-3">
        <label
          htmlFor="questionDescrpition"
          className="block text-sm font-medium text-gray-700"
        >
          Description
        </label>
        <textarea
          name="questionDescrpition"
          id="questionDescrpition"
      ...
      <div>
        {shouldHaveOptions() && (
          <div>
            ...
            {model.data.option.length > 0 && (
              <div>
                {model.data.option.map((op, ind) => (
                  <div className="flex items-center mb-1" key={ind}>
                    ...
                    <input
                      ...
                      value={op.text}
                      onChange={(e) => {
                        const updatedOptions = [...model.data.option];
                        updatedOptions[ind].text = e.target.value;
                        setModel({
                          ...model,
                          data: { ...model.data, option: updatedOptions },
                        });
                      }}
                    />
                    <button
